...
export default function GetRoutePage() {
  ...
  const [usedBoxes, setUsedBoxes] = useState<number>(0);
  ...
  const handleGetOptimizedRoute = async () => {
    ...
    const result = await fetch(
      process.env.NEXT_PUBLIC_BACKEND_URL + "/api/routes/create",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
          ...
          vehicleCapacity: form.capacity,
        }),
      },
    );
    const data = await result.json();
    if (result.ok) {
      router.push(`/optimized-route/${data.route._id}`);
    }
  };
  return (
