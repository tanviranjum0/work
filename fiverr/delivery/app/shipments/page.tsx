...
//         Delete this shipment? This can&apos;t be undone.
//       </p>
...
export default function ShipmentsList() {
  ...
  const getInitialData = async () => {
    const result = await fetch(
      process.env.NEXT_PUBLIC_BACKEND_URL + "/api/shipments?limit=10",
      {
        method: "GET",
  ...
  const handleStatusChange = (id: string, status: ShipmentStatus) => {
    if (status == "deleted") return handleComplete(id);
    ...
    runUpdate(id, async () => {
      ...
      if (result.ok) {
        setShipments((list) =>
          list.map((s) => (s._id === id ? { ...s, status } : s)),
        );
        pushToast(
          "info",
          `${shipment?._id} marked as ${STATUS_META[status].label}.`,
        );
      } else {
        const data = await result.json();
  ...
  //     () => {
  //       setShipments((list) => list.filter((s) => s._id !== id));
  //       pushToast("danger", `${shipment?._id} was deleted.`);
  //     },
  //     400,
  //   );
  // };
      ...
      <div className="page-wrapper">
        <div className="page-inner">
          ...
          <div className="page-header">
            ...
            <Link href={"/new-shipment"} className="btn-new">
