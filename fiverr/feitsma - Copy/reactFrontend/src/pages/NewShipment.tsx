  ...
  return (
    <div className="card">
      ...
      <ReviewRow
        icon={<IconBox />}
        label="Shipment Info"
        value={`${form.boxQuantity} Boxes  |  ${form.shipmentType.toUpperCase()} ${form.isUrgent ? "  | URGENT" : ""} ${form.routeNumber != "" ? " | " + form.routeNumber : ""}`}
      />
...
export default function NewShipment() {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.boxQuantity || !form.shipmentType) {
      setError("Put valid shipment type or quantity");
      setStep(3);
      setLoading(false);
    ...
    const result = await fetch("/api/shipments/create", {
      method: "POST",
