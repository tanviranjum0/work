  ...
  return (
    <div className="summary-row">
      ...
      <span className="summary-value">
        {shipment?.distanceData.distance ?? "Loading…"} km
      </span>
    </div>
  );
}
...
function RouteMapFrame({
  ...
  const mapSrc = useMemo(() => {
    ...
    return `https://www.google.com/maps/embed/v1/directions?origin=${encodeURIComponent(origin)}&destination=${encodeURIComponent(destination)}&key=AIzaSyCvZY0eMYh-qfrBQTfPMs5jveaQjoqbm84`;
...
export default function BestRouteMap() {
  ...
  const loadShipment = async () => {
    const result = await fetch(`/api/shipments/${id}`, {
      method: "GET",
    ...
    if (!result.ok) {
      ...
      if (data.message == "You are not authorized to access this shipment.") {
        alert("You are not authorized to access this shipment.");
        router("/home");
      }
    }
    setShipment(data);
    ...
    <>
      ...
      <div className="page-wrapper">
        <div className="route-card">
          {/* ── Mobile Tabs ── */}
          <div className="mobile-tabs mt-5">
            <button
